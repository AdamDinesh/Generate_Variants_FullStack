...
let btnShowVariant = document.querySelector(".btn-show-variant");
...
btnShowVariant.addEventListener("click", listVariants);
...
///////////////////////////////////////////////////////
// ADD VARIANTS DATA IN DB

function addVariantsDataInDB() {
  let url = `http://localhost:8080/Generate-Variants/data/add`;

  fetch(url, {
    method: "POST",
    body: JSON.stringify({
      variantData: variantArr,
    }),
  })
...
///////////////////////////////////////////////////
/////////////////////////////////////////////////
// GET ALL VARIANTS DATA IN DB (USING FETCH CALL) & LIST VARIANTS (CART ITEMS)
...
function listVariants() {
  ...
  let url = `http://localhost:8080/Generate-Variants/data/list`;

  fetch(url)
    .then((response) => response.json())
    .then((data) => {
      console.log(data);
      if (data.status == "success" && data.variantData) {
        data.variantData.forEach((item, i) => {
